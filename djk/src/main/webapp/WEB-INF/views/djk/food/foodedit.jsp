...
com.djk.web.entity.food.Food fo = (com.djk.web.entity.food.Food)request.getAttribute("food");
%>
 ...
 <script>
    ...
    function setup(){
	   ...
	   /*  1:拿到select对象： var myselect=document.getElementById("test");
	    ...
	    document.getElementById('province').value = '<%=fo.getFirstOriginName()%>';
	    change(1);  
		document.getElementById('city').value='<%=fo.getSecondOriginName()%>';  
		change(2);  
		document.getElementById('county').value='<%=fo.getThreeOriginName()%>';  
    }
    </script>
...
<body>
            <form action="/foodupdate" id="food" name="food" method="post">
        ...
        <div class="container increase_food" uk-grid>
            <div class="mbxue uk-width-1-1">
                ...
                <div class="btn_cz">
                    ...
                    <input class="uk-button uk-button-primary" onclick="onCheckNodeSite()" value="保存"/>
                </div>
            </div>
                    ...
                    <div class="zTreeDemoBackground">
                        <ul class="list">
                            ...
                            <li>
                                ...
                                <input class="uk-input uk-form-width-medium" type="text" name="name" id="name" value="${food.name}">
                                <input type="hidden" name=oldName id="oldName" value="${food.name}">
                                <input type="hidden" name="id" id="id" value="${food.id}">
                            </li>
                            <li>
                                <label><span></span>食物别名</label>
                                <input class="uk-input uk-form-width-medium" type="text" name="alias" value="${food.alias}">
                            </li>
                            <li>
                            ...
                            <li>
                                ...
                                <input class="uk-input uk-form-width-medium citySel in2" onclick="showMenu(2);"type="text" name="secondCategoryName" value="${food.secondCategoryName}">
                                <input type="hidden" name="secondCategoryId" id="secondCategoryId" value="${food.secondCategoryId}">
                            </li>
                            <li>
                                <label><span></span>第三分类</label>
                                <input class="uk-input uk-form-width-medium citySel in3" onclick="showMenu(3);" type="text" name="threeCategoryName" value="${food.threeCategoryName}">
                                <input type="hidden" name="threeCategoryId" id="threeCategoryId" value="${food.threeCategoryId}">
                            </li>
                            <li style=" width: 39%">
                            ...
                            <li>
                                ...
                                <select  class="uk-select" id="form-stacked-select" name="seasonName">
                                     <c:forEach items="${foodSeasonalList}" var="item">
	                                    <option value="${item.seasonal}" <c:if test="${item.seasonal == food.seasonName}">selected</c:if>>${item.seasonal}</option>
							        </c:forEach>
                                </select>
                            </li>
                            <li>
                                ...
                                <select class="uk-select" id="form-stacked-select" name="supplierName">
                                 <c:forEach items="${foodSupplierList}" var="item">
                                    <option value="${item.supplierName}" <c:if test="${item.supplierName == food.supplierName}">selected</c:if>>${item.supplierName}</option>
						        </c:forEach>
                                </select>
                            </li>
                            <li>
                                ...
                                <div class="jldw">
                                    <input class="uk-input" type="text" name="unitNum" value="${food.unitNum}" id="unitNum" onkeyup="value=value.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')">
            ...
            <div class="uk-width-1-1" uk-grid>
                ...
                <div class="right uk-width-2-3">
                        <table class="uk-table uk-table-middle uk-table-divider">
                            ...
                            <tbody>
                             	<c:forEach items="${foodNutritionComponentList}" var="item">
	                                <tr>
	                                    <td>${item.nutritionComponentName}<input type="hidden" name="nutritionName" value="${item.nutritionComponentName}"/></td>
	                                    <td>${item.nutritionComponentUnit}<input type="hidden" name="nutritionUnit" value="${item.nutritionComponentUnit}"/></td>
	                                    <td><input class="uk-input uk-form-blank uk-form-width-medium" type="text" placeholder="" onkeyup="value=value.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')" name="nutritionValue" value="${item.nutritionComponentNum}">
	                                    </td>
	                                </tr>
						        </c:forEach>
                            </tbody>
...
<SCRIPT type="text/javascript">
    ...
    function addDiyDom(treeId, treeNode) {
        ...
        if (btn) btn.bind("change", function() {checkAccessories(treeNode, btn);});
           ...
           /*  var editStr = "<input type='radio' class='uk-radio' id='radio_" +treeNode.id+ "' name='radio_"+treeNode.getParentNode().id+"' onfocus='this.blur();'></input>";
            aObj.before(editStr);
        ...
        if (treeNode.isParent) { //判断是否是父节点
        	...
        	$.ajax({
       	    	type: "get",
       			url : "/findpubliccategorycheck?foodId=<%=fo.getId()%>",
       			data: {},
       	        dataType: "json",
       			success : function(data) {
	     			 ...
	     			 var editStr = "<input type='radio' "+check+" class='uk-radio' id='radio_" +treeNode.id+ "' name='radio_"+treeNode.getParentNode().id+"' value='"+treeNode.id+"' onfocus='this.blur();'></input>";
	 	             aObj.before(editStr);
	 	             var btn = $("#radio_"+treeNode.id);
	 	             if (btn) btn.bind("click", function() {checkBrand(treeNode, btn);});
	       				/* if(data.indexOf(treeNode.id)) {alert(data.indexOf(treeNode.id)+",");
	       	        		check = "checked";
		       	        	 var editStr = "<input type='radio' "+check+" class='uk-radio' id='radio_" +treeNode.id+ "' name='radio_"+treeNode.getParentNode().id+"' value='"+treeNode.id+"' onfocus='this.blur();'></input>";
		       	             aObj.before(editStr);
		       	             var btn = $("#radio_"+treeNode.id);
    ...
    function openPublicCategoryCheck(foodId,foodCategoryId) {
  	  ...
  	  var settingPublic = {
 		        ...
 		        async: {  
 		            enable: true, // 设置 zTree是否开启异步加载模式  加载全部信息  
 		            type: "post",  
 		            url : '/publiccategorycheck/list?id='+foodId+"&foodCategoryId="+foodCategoryId,
 		            autoParam : [ "id", "pid"],
 		            isSimpleData: true, // 简单数据模型，必须提供下面的两项
 		            treeNodeKey: "id", // 节点的id
 		            treeNodeParentKey: "pid",  // 父节点的id
    ...
    $(document).ready(function(){
    	...
    	openPublicCategoryCheck('<%=fo.getId()%>','<%=fo.getFirstCategoryId()%>');
